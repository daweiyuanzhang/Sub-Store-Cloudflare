...
- ✅ Cron Triggers 定时同步
- ✅ 构建期适配 Sub-Store 源码，不改 upstream 仓库
- ✅ GitHub Actions 只监测上游 latest，不直接部署
- ✅ Workers 和 Pages 由 Cloudflare Git 集成 build/deploy，使用 Cloudflare 构建额度
- ✅ 前端 release 自动打上 Cloudflare 版品牌
- ✅ 后端对外识别为 Cloudflare Workers，不再显示 Surge
- ✅ `worker-rs/` 原生 Rust Worker 路线已开始
...
- **worker-rs**: 已加入原生 Cloudflare Worker 入口，目前覆盖 env/status/health 路由；完整 Sub-Store 运行时仍在 JS 兼容层
...
- Pages：绑定这个 GitHub repo，生产分支选择 `main`
...
当前过渡实现仍保留了旧 dashboard 的用户隔离数据结构，但项目方向是单用户自用，不继续扩展多用户管理面板。
...
| `pnpm run build:worker-rs` | dry-run 构建 worker-rs 原生 Worker |
| `pnpm run build:pages` | 构建 Cloudflare Pages 前端产物 |
| `pnpm run dev` | 本地开发服务器 |
| `pnpm run deploy:local` | 从本地部署到 Cloudflare |
| `pnpm run install:backend` | 安装 Sub-Store 后端依赖 |
| `pnpm run fetch:substore` | 下载 Sub-Store 源码 |
